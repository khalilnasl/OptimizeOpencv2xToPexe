...
void LteMacVUeMode4::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        //hashSelection_ = par("hashSelection");
    ...
    else if (stage == inet::INITSTAGE_NETWORK_LAYER_3)
    {
        ...
        // Counter for munber of vehicles
        //std::vector<int> NumSelector;
        //int Selector_ID = getMacNodeId();
        //NumSelector.push_back(Selector_ID);
...
void LteMacVUeMode4::macHandleSps(cPacket* pkt)
{
    ...
    //Sorts the elements(subframes) in the range [first,last) into ascending order
    std::sort(std::begin(CSRs), std::end(CSRs),
    ...
    // Get the selector ID
    int selectorId = getMacNodeId();

    // Adjust the selectorId to start from 0
    // First the platoon vehicles are added to the simulation, the first Mac ID (2097) is assigned to the first platoon vehicle. the second 2098, the third 2099, ...
    int adjustedSelectorId = selectorId - 2097;

    // Check if the Vehicle should use partitioned or random selection
    if (adjustedSelectorId < platoonSize) {
        // Partitioned selection
        int partitionSize = CSRs.size() / platoonSize;
        int startIndex = adjustedSelectorId * partitionSize;
        int endIndex = (adjustedSelectorId == platoonSize - 1) ? (CSRs.size() - 1) : (startIndex + partitionSize - 1);

        int index = intuniform(startIndex, endIndex, 1); 
        selectedCR = CSRs[index];
    } else {
        // Random selection (this part remains unchanged)
